...
class RemindersActivityTest :
    ...
    //On SaveReminder screen, Making sure that the activity is closed before resetting the database.
    @Test
    ...
    //On SaveReminder screen, write a test text in title and description, and make sure the the reminder is now saved
    @Test
    fun reminder_Location_Activity_show_toast_message() {
        ...
        // click on the FAB add reminder, write a test text in title and description
        onView(withId(R.id.addReminderFAB)).perform(click())
        ...
        // click on the selectlocation and open the map fragment
    ...
    // Show a snackbar text if it was an error explanation
    @Test
    fun reminder_Location_Activity_show_Snackbar_message() {
        ...
        // click on the add reminder button, write a test text in description
        onView(withId(R.id.addReminderFAB)).perform(click())
        onView(withId(R.id.reminderDescription)).perform(replaceText("Test Description"))

        Thread.sleep(1000)

        // click on the select location, then map fragment
        onView(withId(R.id.selectLocation)).perform(click())
        onView(withId(R.id.map_fragment)).perform(click())

        Thread.sleep(3000)

        // click on the save reminder location button, then save reminder.
        onView(withId(R.id.save_reminder_location_button)).perform(click())
        onView(withId(R.id.saveReminder)).perform(click())
